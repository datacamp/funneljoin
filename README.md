...
We can use `funnel_start` and `funnel_step` to make an activity funnel. `funnel_start` takes four arugments:
...
If you use a type of afterjoin that allows multiple moments of one type for a user, like "first-any", you will get more rows per user rather than more columns. For example, user 1 had two purchases, so she now has two rows. The `timestamp_landing` and `timestamp_registration` is the same for both rows, but they have a different `timestamp_purchase`.
