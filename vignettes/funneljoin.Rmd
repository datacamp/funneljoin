...
`summarize_conversions` also works if we have multiple types of conversions, specified by a column. Just group by that column before doing `summarize_conversions`. You can also specify the name of your second timestamp column with the argument `time_col_y` (make sure you enter it unquoted). 
